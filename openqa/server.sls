  ...
  ini.options_present:
    - sections:
        ...
        hooks:
          ...
          job_done_hook: env host=openqa.suse.de email_unreviewed=true exclude_name_regex='.*(:investigate:|SAPHanaSR|saptune).*' exclude_group_regex='.*(Development|Public Cloud|Released|Others|Kernel|Virtualization|Containers|BCI).*' grep_timeout=60 nice ionice -c idle /opt/os-autoinst-scripts/openqa-label-known-issues-and-investigate-hook
        influxdb:
